        ...
        db.collection("heThongDanhMuc").doc("danhMucTongPhuongNam").onSnapshot(doc => { 
            if (doc.exists) { 
                ...
                databaseExcel = res.databaseExcel || []; // Lấy lại dữ liệu Excel để làm Checklist
...
function taiDanhMucLinhKienChuand() {
    db.collection("danhMucLinhKien").orderBy("tenLoaiBo").onSnapshot((snapshot) => {
        ...
        // HÀM IN SỔ TAY GIAO NHẬN CHO TAB 1
        function inHoaDonGiaoNhan() {
            const k = document.getElementById("khoa_selKhoa").value;
            if (!k) return showToast("Vui lòng chọn Khoa/Phòng trước khi in!", "error");
            
            let printHtml = `
                <div style="text-align:center; margin-bottom: 20px;">
                    <h2>BIÊN BẢN GIAO NHẬN DỤNG CỤ CSSD</h2>
                    <p>Khoa/Phòng: <strong>${k}</strong> - Ngày xuất phiếu: <strong>${new Date().toLocaleDateString('vi-VN')}</strong></p>
                </div>
                <table style="width:100%; border-collapse: collapse; text-align: left; font-size: 13px;">
                    <thead>
                        <tr>
                            <th style="border: 1px solid #000; padding: 10px;">Phân Loại Mâm / Loại Dụng Cụ</th>
                            <th style="border: 1px solid #000; padding: 10px; text-align: center;">Đã Trả Bẩn</th>
                            <th style="border: 1px solid #000; padding: 10px; text-align: center;">Nhận Sạch</th>
                            <th style="border: 1px solid #000; padding: 10px; text-align: center;">CSSD Nợ Khoa</th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                <div style="display:flex; justify-content: space-between; margin-top: 50px; text-align:center;">
                    <div style="width: 50%;"><strong>ĐẠI DIỆN KHOA LÂM SÀNG</strong><br><i style="font-size: 11px;">(Ký và ghi rõ họ tên)</i><br><br><br><br></div>
                    <div style="width: 50%;"><strong>ĐẠI DIỆN CSSD</strong><br><i style="font-size: 11px;">(Ký và ghi rõ họ tên)</i><br><br><br><br></div>
                </div>
            `;
            const pZone = document.getElementById("print-zone");
            pZone.innerHTML = printHtml;
        ...
        function moPopupKiemDem(id) { 
            ...
            if(item) { 
                document.getElementById("popBo").innerText = item.bo; 
                document.getElementById("popKhoa").innerText = item.khoa; 
                
                // Lấy chi tiết dụng cụ từ Excel đổ vào Checklist Popup
                ...
                if(itemsInBo.length > 0) {
                    chkList.innerHTML = itemsInBo.map((ct, idx) => {
                        let tenDc = ct['Tên Dụng Cụ Chi Tiết'] || ct['Tên dụng cụ'] || ct['Chi tiết'] || ct['Dụng cụ'] || ct['TÊN DỤNG CỤ'] || ct['Tên Chi Tiết'] || "Dụng cụ";
                        let sl = ct['Số lượng'] || ct['SL'] || ct['Số Lượng'] || 1;
                        return `<label class="flex items-center justify-between py-2 border-b border-slate-200 cursor-pointer hover:bg-slate-100 px-2 rounded">
                            <div class="flex items-center gap-2">
                                <input type="checkbox" class="w-4 h-4 rounded text-sky-600">
                                <span class="text-[12px] font-bold text-slate-700">${tenDc}</span>
                            </div>
                            <span class="text-[12px] font-black text-sky-700 bg-sky-100 px-2 py-0.5 rounded">SL: ${sl}</span>
                        </label>`;
                    }).join('');
                } else {
                    chkList.innerHTML = `<p class="italic text-center text-[11px] text-slate-400 py-4">Hệ thống chưa có cấu hình chi tiết bộ dụng cụ này. Vui lòng tự kiểm đếm.</p>`;
                }
        ...
        function chotDongGoi() { if(!idDangDongGoi) return; db.collection("phieuGiaoNhan").doc(idDangDongGoi).update({ status: "CHO_HAP", hsd: document.getElementById("popDG_Han").dataset.dateDB }).then(() => { showToast("Đã đóng gói, chuyển chờ hấp!", "success"); closePopupDongGoi(); callRender(); }); }
        ...
        function inTemTongHangLoat() { let checkboxes = document.querySelectorAll('.hap-checkbox:checked'); if(checkboxes.length === 0) return showToast("Chọn ít nhất 1 mâm để in tem!", "error"); showToast("Đang kết xuất dữ liệu máy in...", "success"); }
        function xacNhanMeHap() { let checkboxes = document.querySelectorAll('.hap-checkbox:checked'); if(checkboxes.length === 0) return showToast("Chọn ít nhất 1 mâm!"); let batchCode = document.getElementById("hap_maySo").value; let p = []; checkboxes.forEach(cb => { if(cb.id === 'selectAllHap') return; p.push(db.collection("phieuGiaoNhan").doc(cb.value).update({status: "DANG_HAP", batchCode: batchCode})); }); Promise.all(p).then(() => { showToast("Đã đưa vào lò hấp!", "success"); callRender(); }); }
        ...
        function renderTheoTabHienTai() {
            if(activeTab === 'khoaphong') {
                ...
                document.getElementById("bangChoThuGom").innerHTML = lsTG.map(i => {
                    // LẤY CHI TIẾT BẢNG CHECKLIST TỪ EXCEL CHO TAB 2
                    ...
                    let checklistHtml = itemsInBo.length > 0 
                        ? `<div class="max-h-24 overflow-y-auto pr-1">` + itemsInBo.map(item => {
                            let tenDc = item['Tên Dụng Cụ Chi Tiết'] || item['Tên dụng cụ'] || item['Chi tiết'] || item['Dụng cụ'] || item['TÊN DỤNG CỤ'] || item['Tên Chi Tiết'] || "Dụng cụ";
                            let sl = item['Số lượng'] || item['SL'] || item['Số Lượng'] || 1;
                            return `<div class="flex justify-between border-b border-dashed border-slate-200 py-1 text-[10px] text-slate-600"><span>- ${tenDc}</span><span class="font-bold text-sky-700">x${sl}</span></div>`;
                        }).join('') + `</div>`
                        : `<span class="italic text-[10px] text-slate-400">Không có cấu hình chi tiết</span>`;

                    return `<tr class="border-b border-slate-50">
                        <td class="p-3">
                            <div class="font-bold text-slate-700 text-[11px] uppercase">${i.khoa}</div>
                            ${i.ghiChu ? `<div class="text-[10px] text-rose-600 font-medium italic mt-1"><i class="fa-solid fa-triangle-exclamation mr-1"></i>${i.ghiChu}</div>` : ''}
                        </td>
                        <td class="p-3">
                            <div class="font-bold text-sky-700 text-[12px] uppercase">${tenBo}</div>
                            <div class="text-[10px] font-mono text-slate-400 mb-2">Mã ID: ${i.maMacDinh}</div>
                            <div class="bg-slate-50 p-2 rounded border border-slate-100">${checklistHtml}</div>
                        </td>
                        <td class="p-3 text-center text-[10px] text-slate-500 font-bold">${i.time}</td>
                        <td class="p-3 text-center action-col"><button onclick="moPopupKiemDem('${i.firestoreId}')" class="bg-sky-600 text-white hover:bg-sky-700 px-3 py-1.5 rounded shadow font-black text-[11px]">KIỂM ĐẾM</button></td>
                    </tr>`;
                }).join('');
            }
        ...
        function renderAdminInterface() {
            ...
            if(tbKhoa) {
                if(danhSachKhoa.length === 0) { tbKhoa.innerHTML = `<tr><td colspan="3" class="p-4 text-center text-rose-500 font-bold italic">Danh sách Khoa đang trống. Vui lòng nạp Excel.</td></tr>`; } 
                    ...
                    tbKhoa.innerHTML = danhSachKhoa.map((k, index) => `
                        <tr class="border-b hover:bg-slate-50">
                            <td class="p-3 font-black text-slate-700 text-[11px]">${k.ten}</td>
                            <td class="p-3 text-center border-x border-slate-200 font-mono font-black text-rose-600 text-sm bg-rose-50/50">${k.pin || '123'}</td>
                            <td class="p-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <input type="text" id="pin-khoa-${index}" placeholder="Nhập PIN mới" class="w-24 p-1.5 text-center border border-slate-300 rounded font-mono font-bold text-sky-700 text-xs">
                                    <button onclick="updatePINTrựcTiep(${index}, '${k.ten}')" class="bg-sky-600 text-white font-bold py-1.5 px-3 rounded shadow hover:bg-sky-700 text-[10px]">ĐỔI PIN</button>
                                </div>
                            </td>
                        </tr>`).join('');
                }
            ...
            if(tbKtv) {
                if(danhSachKtvCssd.length === 0) { tbKtv.innerHTML = `<tr><td colspan="4" class="p-4 text-center text-slate-400 italic">Chưa có Nhân viên nào. Bấm nút Thêm để tạo mới.</td></tr>`; }
                    ...
                    tbKtv.innerHTML = danhSachKtvCssd.map((nv, index) => `
                        <tr class="border-b hover:bg-slate-50">
                            <td class="p-3 font-black text-sky-700 text-xs">${nv.code}</td>
                            <td class="p-3 font-bold text-slate-700 text-[11px]">${nv.ten}</td>
                            <td class="p-3 text-center border-x border-slate-200 font-mono font-black text-sky-600 text-sm bg-sky-50/50">${nv.pin}</td>
                            <td class="p-3 text-center">
                                <button onclick="xoaKtvCssd('${nv.code}')" class="text-rose-600 hover:text-rose-800 font-bold text-[10px]"><i class="fa-solid fa-trash-can mr-1"></i>XÓA NV</button>
                            </td>
                        </tr>`).join('');
                }
            }
        ...
        function updatePINTrựcTiep(idx, tenKhoa) {
            ...
            if(found) {
                ...
                db.collection("heThongDanhMuc").doc("danhMucTongPhuongNam").update({ danhSachKhoa: danhSachKhoa }).then(() => {
                    document.getElementById(`pin-khoa-${idx}`).value = ""; showToast(`Đã đổi PIN cho Khoa ${tenKhoa}!`, "success");
                });
            }
        }
        ...
        function processExcelUpload() {
            ...
            reader.onload = function(e) {
                ...
                excelData.forEach(row => {
                    ...
                    if(k && bo) {
                        k = String(k).trim().toUpperCase(); bo = String(bo).trim();
                        if(!khoaMap[k]) khoaMap[k] = []; if(!khoaMap[k].includes(bo)) khoaMap[k].push(bo);
                    }
                });
...
// Gọi hàm này chung với các hàm tải dữ liệu ban đầu khác của hệ thống
