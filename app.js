...
// Biến cục bộ phục vụ module liên kết sử dụng bệnh nhân tại khoa lâm sàng
...
function taiDanhMucLinhKienChuan() {
    ...
    for (let tenBo in gopBoExcel) {
        ...
        gopBoExcel[tenBo].forEach(item => {
            ...
            let sl = parseInt(item['Số lượng'] || item['SL'] || item['Số Lượng'] || 1) || 0;
    ...
    gioHangXuatKho.forEach(khay => { 
        p.push(
            db.collection("phieuGiaoNhan").doc(khay.firestoreId).update({ 
                ...
                dongBoSangMicrosoft365("XAC_NHAN_XUAT_KHO", { ...khay, status: "ĐANG_VAN_CHUYEN", khoa: k });
            })
        ); 
    });
    Promise.all(p).then(() => { 
        ...
        showToast(`Đã chuyển ${gioHangXuatKho.length} mâm đồ sang trạng thái "Đang vận chuyển" về Khoa ${k}!`, "success"); 
        gioHangXuatKho = []; 
...
function khoaKyNhanDoSachDienTu() {
    ...
    const dsIdDuocChon = [];
    checkboxes.forEach(chk => dsIdDuocChon.push(chk.value));

    if (dsIdDuocChon.length === 0) {
        playSound('error');
        return showToast("Vui lòng tích chọn ít nhất một khay dụng cụ sạch để ký nhận!", "error");
    }

    if (confirm(`Xác nhận ký hồ sơ điện tử tiếp nhận ${dsIdDuocChon.length} mâm dụng cụ sạch về tủ đồ khoa ${khoaDangNhap}?`)) {
        let p = [];
        dsIdDuocChon.forEach(idDoc => {
            ...
            p.push(
                db.collection("phieuGiaoNhan").doc(idDoc).update({
                    status: "HOAN_TAT",
                    nguoiKyNhanKhoa: nameNguoiDung,
                    timeKhoaNhanThucTe: new Date().toLocaleTimeString('vi-VN'),
                    ngayKhoaNhanThucTe: getTodayDateStr()
                }).then(() => {
                    if(itemData) {
                        dongBoSangMicrosoft365("KHOA_KY_NHAN_DO_SACH", { ...itemData, status: "HOAN_TAT" });
                    }
                })
            );
        });
        ...
        Promise.all(p).then(() => {
            playSound('success');
            showToast(`📝 Đã ký tiếp nhận ${dsIdDuocChon.length} mâm đồ sạch thành công vào tủ khoa!`, "success");
            callRender();
        }).catch(err => {
            console.error(err);
            showToast("Lỗi khi cập nhật chữ ký điện tử!", "error");
        });
    }
}
    ...
    checkboxes.forEach(cb => { 
        ...
        p.push(
            db.collection("phieuGiaoNhan").doc(cb.value).update({ 
                ...
                if(itemData) {
                    dongBoSangMicrosoft365("KICH_HOAT_ME_HAP", { ...itemData, batchCode: batchCode, hasBI: coKemBI, thongTinLoHap: thongTinLo });
                }
            })
        ); 
    }); 
...
function hanhDongXemAnhBiMoi(firestoreId) {
    ...
    if (!chuoiAnhBase64) return showToast("Mẻ này kế thừa kết quả sinh học hoặc chưa được tải ảnh minh chứng lên.", "error");
...
function kiemTraQuyenDuyetMeHap() {
    ...
    let checkboxesNghiemThu = document.querySelectorAll('.nghiemthu-checkbox:checked');
    let phaiChoKetQuaBI = false;

    checkboxesNghiemThu.forEach(cb => {
        if (cb.getAttribute('data-hasbi') === 'true') {
            phaiChoKetQuaBI = true; 
        }
    });
...
function nhapKhoHangLoat() { 
    ...
    checkboxes.forEach(cb => { 
        ...
        let khoangCachMiliGiay = bayGio - thoiGianHapXong;
        let soPhutThucTe = Math.floor(khoangCachMiliGiay / (1000 * 60)); 

        let soPhutTinhKPI = soPhutThucTe;
        if(coKemBI === true) {
            soPhutTinhKPI = soPhutThucTe - 20;
            if(soPhutTinhKPI < 0) soPhutTinhKPI = 0; 
        }

        let trangThaiKPI = (Math.floor(soPhutTinhKPI) > 30) ? "KHÔNG ĐẠT" : "ĐẠT";
        ...
        p.push(
            db.collection("phieuGiaoNhan").doc(cb.value).update({
                ...
                dongBoSangMicrosoft365("NGHIEM_THU_DAT_VOHUAN", { 
                    ...
                    ketQuaGiamSatKpi: trangThaiKPI,
                    thongTinLoHap: {
                        ...itemData.thongTinLoHap,
                        giamSatChatLuong: capNhatGiamSat
                    }
                });
            })
        ); 
...
function renderTheoTabHienTai() {
    if(activeTab === 'khoaphong') {
        ...
        if(txtNguoiXacNhan) {
            txtNguoiXacNhan.innerText = `PM-ĐD: ${loginUserCode || "Chưa Đăng Nhập"}`;
        }

        let dsDangVanChuyen = tatCaDonCuaKhoa.filter(x => x.status === "ĐANG_VAN_CHUYEN");
        if(badgeChoNhan) badgeChoNhan.innerText = `${dsDangVanChuyen.length} khay`;

        if(tbodyChoNhan) {
            if(dsDangVanChuyen.length === 0) {
                tbodyChoNhan.innerHTML = `<tr><td colspan="5" class="p-3 text-center text-slate-400 italic">Hiện không có dụng cụ nào đang chuyển về khoa.</td></tr>`;
            } else {
                tbodyChoNhan.innerHTML = dsDangVanChuyen.map(khay => {
                    let tenBoGoc = khay.bo ? String(khay.bo).split(" [ID:")[0] : 'N/A';
                    return `
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="p-2 text-center">
                                <input type="checkbox" value="${khay.firestoreId}" class="w-3.5 h-3.5 text-sky-600 rounded border-slate-300 focus:ring-sky-500 cursor-pointer">
                            </td>
                            <td class="p-2 font-mono font-bold text-slate-700">${khay.maMacDinh || 'N/A'}</td>
                            <td class="p-2 font-semibold text-slate-800">${tenBoGoc}</td>
                            <td class="p-2 text-slate-500 font-medium">${khay.nvXuatKho || '--'}</td>
                            <td class="p-2 text-center"><span class="bg-purple-50 text-purple-700 font-mono text-[10px] px-1.5 py-0.5 rounded font-bold">${khay.batchCode || 'N/A'}</span></td>
                        </tr>
                    `;
                }).join('');
            }
        ...
        uniqueIDs.forEach(ma => {
            ...
            let tenBoGoc = currentTrans.bo ? String(currentTrans.bo).split(" [ID:")[0] : "N/A";
            arrHtml.push(`<tr class="border-b border-slate-100 ${dongColorClass}"><td class="p-3 font-mono text-sky-700 font-bold">${ma}</td><td class="p-3 font-bold text-slate-800">${tenBoGoc}</td><td class="p-3 text-slate-500 text-[11px]">${khoaGiữ}</td><td class="p-3 text-center"><span class="px-2.5 py-1 rounded text-[10px] font-bold ${viTriColor}">${viTriText}</span></td><td class="p-3 text-center font-mono font-bold text-slate-400">${currentTrans.batchCode || 'N/A'}</td><td class="p-3 text-center">${hsdBadget}</td></tr>`);
        });
        ...
        uniqueIDs.forEach(ma => {
            ...
            let tenBoGoc = currentTrans.bo ? String(currentTrans.bo).split(" [ID:")[0] : "N/A";
            arrHtml.push(`<tr class="border-b border-slate-100 font-medium"><td class="p-3 font-mono text-sky-700 font-bold">${ma}</td><td class="p-3 font-bold text-slate-800">${tenBoGoc}</td><td class="p-3 text-center"><span class="px-2.5 py-0.5 rounded text-[10px] font-bold ${viTriColor}">${viTriText}</span></td><td class="p-3 text-center font-black text-amber-700 bg-amber-50/50">${chuKyLo} lần</td></tr>`);
        });
        ...
        if(tbody) {
            tbody.innerHTML = listGiaoDich.map(x => {
                ...
                if(x.ketQuaGiamSatKpi) {
                    let badgeColor = x.ketQuaGiamSatKpi === "ĐẠT" ? "text-emerald-700 bg-emerald-50 border-emerald-200" : "text-rose-700 bg-rose-50 border-rose-200";
                    kpiHtml = `<br><span class="text-[9px] px-1 rounded font-black border mt-1 inline-block ${badgeColor}">KPI: ${x.ketQuaGiamSatKpi} (${x.thoiGianTinhKpiPhut}m)</span>`;
                }

                let thongTinKemTheo = x.nvXuatKho || '<span class="text-slate-300 font-normal">Chưa xuất</span>';
                if (x.nguoiKyNhanKhoa) {
                    thongTinKemTheo += ` ➔ <span class="text-emerald-700 font-semibold">[ĐD: ${x.nguoiKyNhanKhoa}]</span>`;
                }
        ...
        if(safeTbody) {
            ...
            if(dataFiltered.length === 0) { safeTbody.innerHTML = `<tr><td colspan="6" class="p-4 text-center text-rose-500 italic font-bold">Không tìm thấy dữ liệu mâm dụng cụ thuộc mẻ: ${searchInp}</td></tr>`; } 
                ...
                safeTbody.innerHTML = dataFiltered.map(x => {
                    let cleanBo = x.bo ? String(x.bo).split(" [ID:")[0] : "N/A"; let badgeColor = "bg-slate-100 text-slate-700";
                    ...
                    else if(x.status === "ĐANG_VAN_CHUYEN") badgeColor = "bg-purple-500 text-white font-bold";
    ...
    if (f && f.danhSachBo) {
        ...
        f.danhSachBo.forEach(x => {
            if (x) {
                ...
                htmlOptions += `<option value="${chuoiX}">`;
...
async function saveKiemDem() { 
    ...
    currentKiemDemData.linhKienKiemDem.forEach(item => {
        ...
        if (item.tinhTrang === "THIẾU" || item.tinhTrang === "HỎNG" || thuc < chuan) {
...
// [TỐI ƯU MÁY IN BIXOLON SLP-TX403] - Cấu hình layout in cuộn dọc 100% block, size 80mmx50mm chống nhảy trang
function inTemTongHangLoat() { 
    ...
    stylePrint.innerHTML = `
        @media print { 
            ...
            #print-zone { position: absolute; left: 0; top: 0; width: 100%; margin: 0; padding: 0; background: white; } 
            @page { size: 80mm 50mm; margin: 0; } 
            .single-tem { 
                width: 100% !important; height: 50mm !important; page-break-inside: avoid; break-inside: avoid; 
                page-break-after: always; margin: 0 !important; padding: 6px !important; box-sizing: border-box !important;
    ...
    checkboxes.forEach((cb) => { 
        ...
        if(item) { 
            ...
            container.innerHTML += `
                <div class="single-tem" style="font-family: Arial; font-size: 11px; color: #000; background: #fff; border: 1px solid #000;">
                    ...
                    <div style="display: flex; justify-content: space-between; font-weight: bold; font-size: 10px;"><span>SL: ${item.slThucTe || 1}</span></div>
                    <div style="display: flex; justify-content: space-between; border-top: 1px dashed #000; padding-top: 3px; font-size: 10px; margin-top: 2px;"><span>NSX: ${dateHapStr}</span><strong>HSD: ${dateHsdStr}</strong></div>
                    <div style="text-align: center; font-size: 9px; font-family: monospace; font-weight: bold; margin-top: 1px;">Lô: ${batchCode}</div>
                </div>
            `; 
        } 
    }); 
    ...
    checkboxes.forEach(cb => { 
        ...
        if(item) { 
            ...
            JsBarcode(`#barcode-lo-${item.firestoreId}`, cleanId, { format: "CODE128", width: 1.4, height: 32, displayValue: true, fontSize: 10, margin: 2 }); 
        } 
    }); 
...
// [TỐI ƯU MÁY IN BIXOLON SLP-TX403] - Tem Nghiệm thu vô khuẩn chuẩn kích thước cuộn đơn mã vạch
function inTemNghiemThuHangLoat() { 
        ...
        if(item) { 
            ...
            container.innerHTML += `
                <div class="single-tem" style="font-family: Arial; font-size: 11px; color: #000; background: #fff; border: 1px solid #000;">
                    ...
                    <div style="text-align: center; font-weight: bold; font-size: 12px; margin: 1px 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${cleanBo}</div>
                    <div style="text-align: center; margin: 2px 0;"><svg id="barcode-nt-${item.firestoreId}"></svg></div>
                    <div style="display: flex; justify-content: space-between; font-weight: bold; font-size: 10px;"><span>SL: ${item.slThucTe || 1}</span><span style="color: green;">ĐẠT VÔ KHUẨN</span></div>
                    <div style="display: flex; justify-content: space-between; border-top: 1px solid #000; padding-top: 3px; font-size: 10px; margin-top: 2px;"><span>NSX: ${dateHapStr}</span><strong>HSD: ${dateHsdStr}</strong></div>
                    <div style="text-align: center; font-size: 8px; font-weight: bold; font-family: monospace; margin-top: 1px;">BATCH: ${item.batchCode || 'N/A'}</div>
                </div>
            `; 
        } 
    }); 
    ...
    checkboxes.forEach(cb => { 
        ...
        if(item) { 
            let cleanId = item.maMacDinh ? item.maMacDinh.replace(/[^a-zA-Z0-9]/g, "") : "0000"; 
            JsBarcode(`#barcode-nt-${item.firestoreId}`, cleanId, { format: "CODE128", width: 1.4, height: 32, displayValue: true, fontSize: 10, margin: 2 }); 
        } 
    }); 
    setTimeout(() => { window.print(); pZone.classList.add("hidden"); }, 300); 
}
    ...
    reader.onload = function(e) {
        try {
            ...
            jsonData.forEach(row => {
                ...
                let soLuong = parseInt(row['SỐ LƯỢNG'] || row['Số lượng'] || row['SL'] || 1) || 0;
                let tuoiTho = parseInt(row['TUỔI THỌ MẺ HẤP'] || row['Tuổi thọ'] || 100) || 100;
...
// =========================================================================
// [MODULE CÀI ĐẶT MỚI]: LIÊN KẾT MÂM DỤNG CỤ VÀO HỒ SƠ BỆNH ÁN LÂM SÀNG
// =========================================================================
function moPopupSuDungBoDungCu() {
    ...
    if(document.getElementById("sd_khoaBenhNhan")) {
...
function scanKhayVaoSuDung() {
    ...
    // Luồng kiểm tra điều kiện khay: Phải đang ở trạng thái sẵn sàng (CHO_XUAT tại kho hoặc HOAN_TAT tại tủ đồ khoa)
    let khayThucTe = listGiaoDich.find(x => x.maMacDinh === ma && (x.status === "CHO_XUAT" || x.status === "HOAN_TAT"));
    if(!khayThucTe) {
        playSound('error');
        return showToast(`Mã ID ${ma} không tồn tại, đã dùng hoặc chưa đạt chuẩn vô khuẩn!`, "error");
    }
