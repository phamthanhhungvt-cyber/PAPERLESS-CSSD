...
function initRealtimeListeners() {
    ...
    db.collection("he_thong_config").doc("danh_muc_master")
        .onSnapshot((doc) => {
            if (doc.exists) {
                const data = doc.data();
                if (data.danhMucLinhKien && data.danhMucLinhKien.length > 0) {
                    globalData.danhMucLinhKien = data.danhMucLinhKien;
                }
                if (data.danhSachKhoa && data.danhSachKhoa.length > 0) {
                    globalData.danhSachKhoa = data.danhSachKhoa;
                }
...
function capNhatGoiYBoDungCuTheoKhoa(tenKhoa) {
    ...
    const filteredItems = tenKhoa 
        ? (globalData.danhMucLinhKien || []).filter(item => item && item.khoa === tenKhoa)
...
function renderBangCongNoKhoa() {
    ...
    const items = selectedKhoa 
        ? (globalData.danhMucLinhKien || []).filter(i => i && i.khoa === selectedKhoa) 
        : (globalData.danhMucLinhKien || []);

    if (!items || items.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" class="p-4 text-center text-xs text-slate-400">Vui lòng chọn Khoa/Phòng để xem cơ số bộ dụng cụ.</td></tr>`;
        return;
    }
...
function renderBangDanhMucLinhKien() {
    ...
    if (tbody) {
        if (!globalData.danhMucLinhKien || globalData.danhMucLinhKien.length === 0) {
            tbody.innerHTML = `<tr><td colspan="3" class="p-4 text-center text-xs text-slate-400">Chưa có dữ liệu danh mục.</td></tr>`;
        } else {
            tbody.innerHTML = (globalData.danhMucLinhKien || []).map(item => `
                <tr class="border-b hover:bg-slate-50 text-xs">
                    ...
                    <td class="p-3 text-center font-bold text-sky-700 bg-sky-50 rounded">${item.soLuong || 1}</td>
                </tr>
            `).join('');
        }
    }
        ...
        if (!globalData.danhMucLinhKien || globalData.danhMucLinhKien.length === 0) {
            ...
            tbodyTong.innerHTML = (globalData.danhMucLinhKien || []).map(item => `
                <tr class="border-b hover:bg-slate-50 text-xs">
                    ...
                    <td class="p-3 text-slate-600">${item.khoa || ''}</td>
                    <td class="p-3 text-center"><span class="bg-emerald-100 text-emerald-800 font-bold px-2 py-0.5 rounded-full text-[10px]">Sẵn Sàng (${item.soLuong || 1})</span></td>
                </tr>
            `).join('');
        }
    }
...
function renderBangTonKhoRealtime() {
    ...
    if (selectedKhoa) {
        items = items.filter(i => i && i.khoa === selectedKhoa);
    }
...
function renderBangKhoVoKhuan() {
    ...
    globalData.khoVoKhuan.forEach(item => {
        ...
        if (expDate && !isNaN(expDate.getTime())) {
            ...
            item._diffDays = diffDays;
            item._expTimestamp = expDate.getTime();
        } else {
            item._diffDays = 999;
            item._expTimestamp = 9999999999999;
        }
    });
    ...
    tbody.innerHTML = globalData.khoVoKhuan.map((item, idx) => {
        ...
        if (diffDays < 0) {
            ...
            badgeHSD = `
                <span class="inline-flex items-center gap-1 bg-rose-100 text-rose-800 text-[11px] font-bold px-2 py-0.5 rounded-md border border-rose-200">
                    ...
                    ĐÃ HẾT HẠN (${item.hanSuDung || 'N/A'})
                </span>
            `;
            actionBtn = `
                <button onclick="thuHoiMamHetHanVeRua(${idx})" class="bg-rose-600 hover:bg-rose-700 text-white font-bold px-2.5 py-1 rounded text-xs shadow-sm transition-all whitespace-nowrap" title="Mâm đã hết hạn - Thu hồi về bồn rửa">
                    <i class="fa-solid fa-rotate-left mr-1"></i> Về Làm Sạch
                </button>
            `;
        } else if (diffDays <= 7) {
            ...
            badgeHSD = `
                <span class="inline-flex items-center gap-1 bg-amber-100 text-amber-900 text-[11px] font-bold px-2 py-0.5 rounded-md border border-amber-300 animate-pulse">
                    ...
                    Còn ${diffDays} ngày (${item.hanSuDung})
                </span>
                <span class="block text-[10px] text-amber-700 font-semibold mt-0.5">⚠️ Ưu tiên xuất FEFO</span>
            `;
            actionBtn = `
                <button onclick="xuatKhoDungCu(${idx})" class="bg-amber-600 hover:bg-amber-700 text-white font-bold px-3 py-1 rounded text-xs shadow-sm transition-all whitespace-nowrap">
                    <i class="fa-solid fa-truck-fast mr-1"></i> Xuất Trả
                </button>
            `;
        } else {
            // Còn hạn dài (> 7 ngày)
            ...
            `;
            actionBtn = `
                <button onclick="xuatKhoDungCu(${idx})" class="bg-sky-600 hover:bg-sky-700 text-white font-bold px-3 py-1 rounded text-xs shadow-sm transition-all whitespace-nowrap">
                    Xuất Trả
                </button>
            `;
        }
        ...
        return `
            <tr class="border-b hover:bg-slate-50 text-xs transition-colors ${diffDays < 0 ? 'bg-rose-50/50' : (diffDays <= 7 ? 'bg-amber-50/40' : '')}">
                <td class="p-3">
                    <div class="font-bold text-slate-800">${item.tenBo || ''}</div>
                    <div class="text-[10px] text-slate-400 mt-0.5">${item.vatLieuBaoBoc || 'Bao gói tiêu chuẩn'}</div>
                </td>
                <td class="p-3 font-mono font-bold text-sky-700">${item.maBo || ''}</td>
                <td class="p-3 text-center text-slate-600 font-semibold">${item.khoa || 'Phòng Sanh'}</td>
                <td class="p-3 text-center">
                    <span class="bg-slate-100 text-slate-700 font-semibold px-2 py-0.5 rounded text-[11px]">
                        ${item.viTriKho || 'Kệ A1'}
                    </span>
                </td>
                <td class="p-3 text-center">
...
function thuHoiMamHetHanVeRua(idx) {
    ...
    const item = globalData.khoVoKhuan[idx];
    if (confirm(`🚨 MÂM HẾT HẠN VÔ KHUẨN: Bạn có chắc chắn muốn thu hồi mâm [${item.tenBo} - ${item.maBo}] về Trạm Rửa để xử lý lại từ đầu?`)) {
        const removed = globalData.khoVoKhuan.splice(idx, 1)[0];
        
        if (!globalData.choRua) globalData.choRua = [];
        
        const newItem = {
            ...removed,
            trangThai: 'HẾT HẠN - THU VỀ RỬA LẠI',
        ...
        ghiNhatKyFirebase({
            ...
            tenBo: removed.tenBo,
            khoa: removed.khoa,
            trangThai: 'QUÁ HẠN DÙNG - THU HỒI TÁI XỬ LÝ',
            maLoHap: removed.maLoHap || '---'
        });

        dongBoTrangThaiRealtime();
        renderBangKhoVoKhuan();
        alert(`🔄 Đã chuyển mâm [${removed.tenBo}] về Hàng Đợi Rửa thành công!`);
    }
}
...
function renderBangKPIPerformance() {
    ...
    const ktvStats = (globalData.ktvList || []).map((ktv, idx) => {
        const logsOfKtv = (globalData.lichSu || []).filter(l => l && l.nhanSu === ktv.name);
        ...
        const totalBiReads = (globalData.meHap || []).filter(m => m && (m.nhanSuHap === ktv.name || m.nhanSu === ktv.name)).length;
        const passBiReads = (globalData.meHap || []).filter(m => m && (m.nhanSuHap === ktv.name || m.nhanSu === ktv.name) && m.kpiBiStatus === 'ĐẠT (<30m)').length;
...
function renderDanhSachPinAdmin() {
    ...
    const filteredList = (globalData.ktvList || []).filter(ktv => 
        ktv && (ktv.name.toLowerCase().includes(keyword) || ktv.id.toLowerCase().includes(keyword))
    );
...
async function chotDongGoi() {
    if (itemDongGoiHienTai === null || !(globalData.choDongGoi || [])[itemDongGoiHienTai]) return;

    const btnConfirm = document.querySelector('#popupDongGoi button[onclick="chotDongGoi()"]');
    if (btnConfirm) {
        btnConfirm.disabled = true;
        btnConfirm.innerHTML = `<i class="fa-solid fa-spinner fa-spin mr-1"></i> Đang lưu...`;
    }
    ...
    if (item) {
        ...
        if (storage && anhBangChungDongGoiTam) {
            try {
                const storageRef = storage.ref();
                const fileName = `evidence_${item.maBo}_${Date.now()}.jpg`;
                const fileRef = storageRef.child(`cssd_evidence/${fileName}`);
                await fileRef.putString(anhBangChungDongGoiTam, 'data_url');
                evidenceUrl = await fileRef.getDownloadURL();
            } catch (err) {
                console.warn("Storage upload bypass:", err);
                evidenceUrl = anhBangChungDongGoiTam;
            }
        }
        ...
        ghiNhatKyFirebase({
            ...
            maLoHap: '---',
            anhBangChung: evidenceUrl
        });
    }

    anhBangChungDongGoiTam = null;
    dongBoTrangThaiRealtime();
    ...
    if (btnConfirm) {
        ...
        btnConfirm.innerHTML = `<i class="fa-solid fa-box-archive mr-1"></i> Xác Nhận Đóng Gói & Chuyển Hấp`;
    }
}
...
function capNhatDoiSoatBangAI(detections, tbodyLinhKien) {
    ...
    if (detections.length === 0) {
        alert("⚠️ AI chưa nhận diện được dụng cụ nào trong khung hình.");
    } else if (soMonThieu > 0) {
        alert(`⚠️ AI đã phát hiện ${detections.length} dụng cụ, mâm vẫn còn THIẾU ${soMonThieu} chi tiết.`);
    } else {
...
function xuatKhoXoayVong() {
    ...
    const idx = (globalData.khoVoKhuan || []).findIndex(i => i && i.maBo.toUpperCase() === ma);
    if (idx !== -1) {
        xuatKhoDungCu(idx);
        inp.value = '';
    } else {
...
function themVaoGio() {
    ...
    const item = (globalData.danhMucLinhKien || []).find(i => i && i.maBo.toUpperCase() === maBoInput) || {
        maBo: maBoInput,
...
function checkLogin() {
    ...
    let foundUser = (globalData.ktvList || []).find(k => k && k.pin === pin);
...
function scanKhayVaoSuDung() {
    ...
    const item = (globalData.danhMucLinhKien || []).find(i => i && i.maBo.toUpperCase() === maKhay) || { maBo: maKhay, tenBo: "Khay Dụng Cụ" };
...
function truyVetTheoMaBatch() {
    ...
    const res = (globalData.lichSu || []).filter(l => l && l.maLoHap && l.maLoHap.toUpperCase().includes(maBatch));
    if (res.length === 0) {
...
function truyVetKhanCapLoHap() {
    ...
    (globalData.dangHap || []).forEach(item => {
        if (item && item.maLoHap && item.maLoHap.toUpperCase() === currentRecallBatchId) {
...
function capNhatPinNhanVien(ktvId) {
    ...
    const targetKtv = (globalData.ktvList || []).find(k => k && k.id === ktvId);
    if (targetKtv && pinInp) {
